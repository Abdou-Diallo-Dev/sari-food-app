-- Bug racine du décaissement qui échoue toujours pour un compte manager,
-- malgré les correctifs RLS de 0042/0043 (confirmés corrects en base par
-- l'utilisateur) : le problème n'était pas RLS mais le trigger
-- verifier_transition_demande(), qui a autorité indépendante sur les
-- transitions de statut (voir commentaire de 0010).
--
-- 0011 avait déjà corrigé la branche 'decaissee' pour autoriser
-- is_manager_or_admin() (cohérent avec le rôle manager = "accès de
-- couverture" pouvant remplacer un comptable). Mais 0026, en recréant toute
-- la fonction pour ajouter le droit du chef de modifier sa propre demande,
-- a réintroduit par erreur l'ancienne condition de 0010 :
-- `my_role() = 'comptable' or is_admin()`, perdant à nouveau le manager.
-- Comme create or replace function écrase entièrement le corps précédent,
-- c'est cette version régressée qui était active — d'où l'échec systématique
-- (exception Postgres "Seul le comptable peut décaisser une demande.",
-- remontée côté app comme l'erreur générique Server Components) malgré une
-- policy RLS pourtant correcte.
--
-- Corps identique à 0026, seule la ligne de la branche 'decaissee' change.
...
begin
  ...
  if new.statut = 'validee' then
    ...
    if not (my_role() = 'comptable' or is_manager_or_admin()) then
      raise exception 'Seul le comptable ou un manager peut décaisser une demande.';
    end if;
